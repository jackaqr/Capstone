# -*- coding: utf-8 -*-
"""
Created on Fri Sep 27 01:41:09 2024
...
class OFDMDetector_0(Layer):
    ...
    * **x_hat** ([..., num_streams_per_rx, num_points], tf.float) or ([..., num_streams_per_rx], tf.int) -- Logits or hard-decisions for constellation symbols for every stream, if ``output`` equals `"symbol"`. Hard-decisions correspond to the symbol indices.
    ...
    def call(self, inputs):
        ...
        ### Detection
        #################################
        z,v_dsc_prev = self._detector([y_dt, h_dt_desired, s])
...
class OFDMDetectorWithPrior_0(OFDMDetector_0):
    # pylint: disable=line-too-long
    ...
    * **x_hat** ([..., num_streams_per_rx, num_points], tf.float) or ([..., num_streams_per_rx], tf.int) -- Logits or hard-decisions for constellation symbols for every stream, if ``output`` equals `"symbol"`. Hard-decisions correspond to the symbol indices.
    ...
    def call(self, inputs):
        ...
        #################################
        z, v_dsc_prev, x_bse_prev, v_bse_prev, ise_dsc_prev, x_dsc_prev = self._detector([y_dt, h_dt_desired, s])
        ...
        return z,v_dsc_prev, x_bse_prev, v_bse_prev, ise_dsc_prev, x_dsc_prev 


class MaximumLikelihoodDetector(OFDMDetector_0):
    # pylint: disable=line-too-long
...
class MaximumLikelihoodDetectorWithPrior(OFDMDetectorWithPrior_0):
    # pylint: disable=line-too-long
...
class LinearDetector(OFDMDetector_0):
    # pylint: disable=line-too-long
...
class KBestDetector(OFDMDetector_0):
    # pylint: disable=line-too-long
...
class EPDetector(OFDMDetector_0):
    # pylint: disable=line-too-long
...
class MMSEPICDetector(OFDMDetectorWithPrior_0):
    # pylint: disable=line-too-long
