# -*- coding: utf-8 -*-
"""
Created on Wed Sep 25 14:27:15 2024
...
class OFDMDetectorWithPrior_b(OFDMDetector_b):
    # pylint: disable=line-too-long
    ...
    * **x_hat** ([..., num_streams_per_rx, num_points], tf.float) or ([..., num_streams_per_rx], tf.int) -- Logits or hard-decisions for constellation symbols for every stream, if ``output`` equals `"symbol"`. Hard-decisions correspond to the symbol indices.
    ...
    def call(self, inputs):
        y, h_hat, prior, err_var, no, it, v_dsc_prev, x_bse_prev, v_bse_prev, ise_dsc_prev, test,x_dsc, internal_it  = inputs
        # y has shape:
        # [batch_size, num_rx, num_rx_ant, num_ofdm_symbols, fft_size]
        ...
        #print("shape of templet",template.shape)
        # [num_tx, num_streams_per_tx, num_data_symbols,
        #   ... num_bits_per_symbol/num_points, batch_size]
        prior = tf.transpose(prior, [1, 2, 3, 4, 0])
        #print("shape of prior(after transpose)",prior.shape)
        # [num_tx, num_streams_per_tx, num_ofdm_symbols,...
        #   ... num_effective_subcarriers, num_bits_per_symbol/num_points,...
        #   ... batch_size]
        prior = flatten_dims(prior, 3, 0)
        #print("shape of prior(after flatten_dims(prior, 3, 0)",prior.shape)
        ...
        #print("shape of prior(after flatten_dims(prior, 2, 3)",prior.shape)
        ...
        #################################
        z,v_dsc_prev, x_bse_prev, v_bse_prev, ise_dsc_prev, test,x_dsc  = self._detector([y_dt, h_dt_desired, prior, s, it, v_dsc_prev, x_bse_prev, v_bse_prev, ise_dsc_prev, test,x_dsc,internal_it])
        ...
        return z,v_dsc_prev, x_bse_prev, v_bse_prev,ise_dsc_prev, test,x_dsc


class MaximumLikelihoodDetector(OFDMDetector_b):
    # pylint: disable=line-too-long
...
class MaximumLikelihoodDetectorWithPrior(OFDMDetectorWithPrior_b):
    # pylint: disable=line-too-long
...
class LinearDetector(OFDMDetector_b):
    # pylint: disable=line-too-long
...
class KBestDetector(OFDMDetector_b):
    # pylint: disable=line-too-long
...
class EPDetector(OFDMDetector_b):
    # pylint: disable=line-too-long
...
class MMSEPICDetector(OFDMDetectorWithPrior_b):
    # pylint: disable=line-too-long
